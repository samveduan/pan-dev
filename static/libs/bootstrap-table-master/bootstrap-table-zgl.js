...
//            });
            $search.bind('keyup drop',function(event){
                clearTimeout(timeoutId); // doesn't matter if it's 0
                timeoutId = setTimeout(function () {
                    that.onSearch(event);
                }, that.options.searchTimeOut);
        ...
        if (this.options.sidePagination !== 'server') {
            ...
            this.data = s ? $.grep(this.data, function (item, i) {
                for (var key in item) {
                    ...
                    var columnspan=value+"";
                    columnspan=columnspan.replace("<span","").replace("</span>","").replace("title","");
                    if (index !== -1 && that.header.searchables[index] &&
                        (typeof value === 'string' ||
                            typeof value === 'number') &&
                        (columnspan + '').toLowerCase().indexOf(s) !== -1) {
                        return true;
                    }
                }
                return false;
