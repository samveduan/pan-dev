...
function getSelectEdatas(){
	var tu = $("#userinfo-table").bootstrapTable();
...
function getDepts(){
	var url = "/kmjadm/um/user_info/dept/";
	var data = {'type' : 1};
...
$(document).ready(function() {
	var deptnodes = getDepts();
	$.fn.zTree.init($("#tree"), setting, deptnodes);
	$('#btnLocalRecover').bind('click', function() {
		var sltuids = getSelectUids();
		if (sltuids.length == 0) {
//			toastr.error("请选择要恢复的用户！", "错误提示");
			myAlert('请选择要恢复的用户！',"error");
			return false;
		}
		if (sltuids.length > 1) {
//			toastr.error("一次只能恢复的一个用户！", "错误提示");
			myAlert('一次只能恢复的一个用户！',"error");
			return false;
		}
		var suid = sltuids[0];
		$('#ksuid').val(suid);
...
//					toastr.error("挂载失败，" + obj['errMsg'], "错误提示");
				...
				var mdeptnodes = [{'isParent':true,'name':'用户主目录', 'path':'user'+suid}]
...
//				toastr.success("恢复文件及文件夹成功！", "成功提示");
				myAlert('恢复文件及文件夹成功！',"info");
			},
			error : function() {
				$('#moveUserModal').modal('hide');
				$(".shade").hide();
